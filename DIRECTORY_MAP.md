...
│  │  ├─ w[key]       ← 🔒 세계관 페이지 〈사자의 서〉= **아벨 연구소 그 자체**. 알림톡 링크로만 들어온다.
│  │  │                   3층 구조: 위장 홍보 사이트 → 위험 테이프 → 내부망(대외비 문서고).
│  │  │                   열쇠는 lib/world.ts. 틀리면 404. sitemap 제외·robots 금지·noindex.
│  │  │                   /w/ 에선 판타스트릭 헤더·푸터·챗봇·공지팝업이 렌더되지 않는다.
