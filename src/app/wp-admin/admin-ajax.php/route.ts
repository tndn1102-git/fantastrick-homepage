 ...
 *    방화벽(WAF) 규칙에서도 이 주소만 빼야 한다 — `node scripts/waf-allow-ajax.mjs --apply`
...
import {
  CALENDARS, themeOfCalendar, openTimes, openTimesRange, apptListHtml, monthHtml, kstToday,
...
async function handle(req: NextRequest) {
  ...
  /* ── 그 밖 ──
     지금 수집기는 아무 값도 안 붙인 GET 을 하루 5,900번 보낸다. 옛 워드프레스는 여기서
     "0" 한 글자를 돌려줬고, 그 뒤에 제대로 된 요청을 보내던 구조다. 그대로 흉내낸다. */
  return new NextResponse("0", { status: 200, headers: HTML });
}
