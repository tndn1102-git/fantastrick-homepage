...
function PayQueue({ onDone }: { onDone: () => void }) {
  ...
  /* 입금 대기 건 취소 — 30분을 기다리지 않고 **사장님이 먼저 내리는** 수단.
     (2026-08-17 자동취소를 켰지만, 손님이 전화로 "안 갈게요" 하는 경우가 있어 그대로 둔다)
...
입금 전이라 환불할 금액은 없습니다.`)) return;
  ...
  return (
    <>
      ...
      {/* 🔴 오래 방치된 입금 대기 — 안전망 (2026-08-13 도입, 08-17 성격 바뀜)
          자동취소를 꺼뒀던 동안에는 "입금 안 한 예약이 자리를 영영 차지"하는 걸 알리는 장치였다.
          2026-08-17 자동취소를 켠 뒤로는 **30분이면 시스템이 지우므로 여기 걸릴 일이 없다.**
          그래서 이 칸이 보인다 = **정리가 안 돌고 있다는 신호**다(4곳 중 아무 데도 호출이 안 됐다는 뜻).
          지우지 말고 남겨 둘 것 — 고장을 알려주는 표시로 값이 더 커졌다.
          ⚠️ 접수 1시간이 기준이다(마감 30분 + 여유 30분). */}
      {(() => {
